    ...
    def filter_files(self):
        ...
        for img_path, gt_path in zip(self.images, self.gts):
            ...
            if img.size == gt.size:
                ...
                print(f"过滤掉的文件: {img_path} 和 {gt_path}")
        self.images = images
        self.gts = gts
...
def image_loader(image_root, gt_root, val_img_root, val_gt_root, batch_size, image_size, split=1, labeled_ratio=0.05,
                 mode='weak_1'):
    def sorted_files(directory, extensions=['.jpg', '.png']):
        return sorted(
            [os.path.join(directory, f) for f in os.listdir(directory) if os.path.splitext(f)[1] in extensions],
            key=lambda x: os.path.splitext(os.path.basename(x))[0])

    images = sorted_files(image_root)
    gts = sorted_files(gt_root)

    val_img = sorted_files(val_img_root)
    val_label = sorted_files(val_gt_root)
    ...
    print("labeled_train_images_1:", len(labeled_train_images_1))

    labeled_train_gts = train_gts[0:int(len(train_gts) * labeled_ratio)]
    # labeled_train_gts_1 = labeled_train_gts[0:int(len(labeled_train_gts) * 0.5)]
    labeled_train_gts_1 = labeled_train_gts
    print("labeled_train_gts_1:", len(labeled_train_gts_1))

    # labeled_train_images_2 = train_images[0:int(len(train_images) * labeled_ratio)]+ train_images[int(472):]
    labeled_train_images_2 = labeled_train_images_1
    print("labeled_train_images_2:", len(labeled_train_images_2))
    # labeled_train_images_2 = labeled_train_images
    # labeled_train_gts_2 = train_gts[0:int(len(train_gts) * labeled_ratio)]+ train_gts[int(472):]
    labeled_train_gts_2 = labeled_train_gts_1
    # labeled_train_gts_2 = labeled_train_gts
    unlabeled_train_images = train_images[0:int(472)]
    unlabeled_train_gts = train_gts[0:int(472)]
    unlabeled_train_images_2 = train_images
    unlabeled_train_gts_2 = train_gts

    labeled_train_dataset_1 = ObjDataset(labeled_train_images_1, labeled_train_gts_1, image_size, mode='weak')

    print("labeled_train_dataset_1:", len(labeled_train_dataset_1))
    ...
    unlabeled_train_dataset_2 = ObjDataset(unlabeled_train_images_2, unlabeled_train_gts_2, image_size, mode='strong')
    val_dataset = ValObjDataset(val_images, val_gts, image_size)

    print("Training images1:")
    for img_name in labeled_train_images_1:
        print(img_name)
    print("Training images2:")
    for img_name in labeled_train_images_2:
        print(img_name)
    print("unlabeled_train_images:")
    for img_name in unlabeled_train_images:
        print(img_name)
    print("unlabeled_train_images_2:")
    for img_name in unlabeled_train_images_2:
        print(img_name)
    print("\nValidation images:")
    ...
    unlabeled_data_loader_2 = data.DataLoader(dataset=unlabeled_train_dataset_2,
                                              batch_size=batch_size,
                                              num_workers=1,
                                              pin_memory=True,
                                              shuffle=True)
    ...
    return labeled_data_loader_1, labeled_data_loader_2, unlabeled_data_loader, unlabeled_data_loader_2, val_loader